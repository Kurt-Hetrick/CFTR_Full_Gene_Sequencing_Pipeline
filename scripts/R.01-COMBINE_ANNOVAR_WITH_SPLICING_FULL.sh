...
# INPUT VARIABLES
	...
	PROJECT=$3
	SM_TAG=$4
	COMBINE_ANNOVAR_WITH_SPLICING_R_SCRIPT_FULL=$5
	SAMPLE_SHEET=$6
...
START_COMBINE_ANNOTATIONS=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD="singularity exec $COMBINE_ANNOVAR_WITH_SPLICING_R_CONTAINER Rscript" \
			CMD=$CMD" $COMBINE_ANNOVAR_WITH_SPLICING_R_SCRIPT_FULL" \
			...
			CMD=$CMD" $CORE_PATH/$PROJECT/$SM_TAG/ANALYSIS/$SM_TAG".combined_splicing_with_annovar_full""
