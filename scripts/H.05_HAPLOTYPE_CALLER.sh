...
## -----Haplotype Caller -----
## CFTR target region is padded by 250 bp in fix bed files.
## everything else (the barcode snp regions) are not padded.
...
# construct command line
	...
	CMD=$CMD" -T HaplotypeCaller" \
		...
		CMD=$CMD" -L $CORE_PATH/$PROJECT/TEMP/$SM_TAG"-"$BAIT_BED_NAME"-"$GVCF_PAD"-BP-PAD-GVCF.bed"" \
		...
		CMD=$CMD" -bamout $CORE_PATH/$PROJECT/TEMP/$SM_TAG".HC.bam"" \
		CMD=$CMD" --emitDroppedReads" \
		CMD=$CMD" -o $CORE_PATH/$PROJECT/$SM_TAG/GVCF/$SM_TAG".g.vcf.gz""
