...
START_ANNOTATE_HGVS_CDNA=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD=$CMD" -T VariantAnnotator" \
			...
			CMD=$CMD" --variant $CORE_PATH/$PROJECT/TEMP/$SM_TAG".CFTR_REGION_VARIANT_ONLY.DandN.CFTR2.vcf.gz"" \
			CMD=$CMD" -L $CORE_PATH/$PROJECT/TEMP/$SM_TAG".CFTR_REGION_VARIANT_ONLY.DandN.CFTR2.vcf.gz"" \
