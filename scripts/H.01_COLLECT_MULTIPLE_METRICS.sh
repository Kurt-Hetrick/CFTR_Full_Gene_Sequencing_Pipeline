...
START_COLLECT_MULTIPLE_METRICS=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD=$CMD" CollectMultipleMetrics" \
			CMD=$CMD" INPUT=$CORE_PATH/$PROJECT/$SM_TAG/CRAM/$SM_TAG".cram"" \
...
# move and rename output
	...
	# Move and rename pre adapter metrics/summary files to the reports directory and add a txt extension
		...
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/PRE_ADAPTER/METRICS/$SM_TAG".pre_adapter_detail_metrics.txt"

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".pre_adapter_summary_metrics" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/PRE_ADAPTER/SUMMARY/$SM_TAG".pre_adapter_summary_metrics.txt"
	...
	# move the base distribution by cycle reports into the report directory
		...
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/BASE_DISTRIBUTION_BY_CYCLE/PDF/$SM_TAG".base_distribution_by_cycle.pdf"

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".base_distribution_by_cycle_metrics" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/BASE_DISTRIBUTION_BY_CYCLE/METRICS/$SM_TAG".base_distribution_by_cycle_metrics.txt"

	# move the insert size reports into the report directory

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".insert_size_histogram.pdf" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/INSERT_SIZE/PDF/$SM_TAG".insert_size_histogram.pdf"
	...
	# move the mean quality by cycle into the report directory
		...
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/MEAN_QUALITY_BY_CYCLE/PDF/$SM_TAG".quality_by_cycle.pdf"

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".quality_by_cycle_metrics" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/MEAN_QUALITY_BY_CYCLE/METRICS/$SM_TAG".quality_by_cycle_metrics.txt"

	# move the basecall by q score into the report directory

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".quality_distribution.pdf" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/BASECALL_Q_SCORE_DISTRIBUTION/PDF/$SM_TAG".quality_distribution.pdf"

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".quality_distribution_metrics" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/BASECALL_Q_SCORE_DISTRIBUTION/METRICS/$SM_TAG".quality_distribution_metrics.txt"

	# move the gc bias reports into the report directory

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".gc_bias.pdf" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/GC_BIAS/PDF/$SM_TAG".gc_bias.pdf"

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".gc_bias.detail_metrics" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/GC_BIAS/METRICS/$SM_TAG".gc_bias.detail_metrics.txt"

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".gc_bias.summary_metrics" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/GC_BIAS/SUMMARY/$SM_TAG".gc_bias.summary_metrics.txt"

	# move the quality yield report into the report directory

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".quality_yield_metrics" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/QUALITY_YIELD/$SM_TAG".quality_yield_metrics.txt"

	# move the error summary report into the report directory

		mv -v $CORE_PATH/$PROJECT/TEMP/$SM_TAG".error_summary_metrics" \
		$CORE_PATH/$PROJECT/$SM_TAG/REPORTS/ERROR_SUMMARY/$SM_TAG".error_summary_metrics.txt"
