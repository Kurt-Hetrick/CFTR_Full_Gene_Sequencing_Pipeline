...
# RUN md5sum in parallel and write to file
	...
	{
		find ${GIT_LFS_DIR}/CFTR_Full_Gene_Sequencing_Pipeline/GRCh37 -type f \
			| cut -f 2 \
...
# if a hash/filename/subfolder combo is not seen twice then output what files are seen either once or more than twice.
	...
	cat \
	${CORE_PATH}/${PROJECT}/REPORTS/md5_pipeline_resources_${PROJECT}_${SUBMIT_STAMP}.txt \
	${GIT_LFS_DIR}/CFTR_Full_Gene_Sequencing_Pipeline_md5.txt \
		| awk 'BEGIN {OFS="\t"} \
			{print $1,$2}' \
		| awk 'BEGIN {FS="/"} \
			{print $(NF-1) "/" $NF "\t" $0}' \
	...
	>| ${CORE_PATH}/${PROJECT}/REPORTS/md5_diff_CFTR_Full_Gene_Sequencing_Pipeline_${SUBMIT_STAMP}.txt
