...
# remove spanning deletions, rsid from gatk vcf, index with tabix

START_REMOVE_ID=`date '+%s'` # capture time process starts for wall clock tracking purposes.
...
END_REMOVE_ID=`date '+%s'` # capture time process ends for wall clock tracking purposes.

# write out timing metrics to file

	echo $PROJECT",N.001,REMOVE_ID,"$HOSTNAME","$START_REMOVE_ID","$END_REMOVE_ID \
