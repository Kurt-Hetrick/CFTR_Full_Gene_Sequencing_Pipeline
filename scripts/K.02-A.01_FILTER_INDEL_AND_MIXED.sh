...
# APPLY HARD FILTERS TO INDEL AND MIXED
...
# construct command line
	...
	CMD=$CMD" -T VariantFiltration" \
		...
		CMD=$CMD" --variant $CORE_PATH/$PROJECT/TEMP/$SM_TAG.RAW.ANNOTATED.INDEL_MIXED.vcf.gz" \
		CMD=$CMD" -o $CORE_PATH/$PROJECT/TEMP/$SM_TAG.FILTERED.INDEL_MIXED.vcf.gz" \
		CMD=$CMD" --filterExpression 'QD < 2.0'" \
		CMD=$CMD" --filterName 'QD_filter_INDEL'" \
		CMD=$CMD" --filterExpression 'ReadPosRankSum < -20.0'" \
		CMD=$CMD" --filterName 'ReadPosRankSum_INDEL'" \
		CMD=$CMD" --filterExpression 'FS > 200.0'" \
		CMD=$CMD" --filterName 'FSfilter_INDEL'" \
		CMD=$CMD" --filterExpression 'QUAL < 30.0'" \
		CMD=$CMD" --filterName 'QUALfilter_INDEL'" \
...
# write out timing metrics to file
	...
	echo $SM_TAG"_"$PROJECT",L.001,FILTER_INDEL_AND_MIXED,"$HOSTNAME","$START_FILTER_INDEL_AND_MIXED","$END_FILTER_INDEL_AND_MIXED \
